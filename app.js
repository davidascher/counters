...
app.get('/:name', function(req, res) {
  redis.get('counter-'+req.params.name, function(err, result) {
    ...
    res.send("<link rel='stylesheet' type='text/css' href='main.css' /><h1>Counting: " + result + ' ' + req.params.name +'s' + '</h1><p>Counter '+ req.params.name + ' was hit <span class="counter">' + result + '</span> times. <a href="">Reloading this page</a> won\'t change that.  <a href="/add/' + req.params.name +'">hit me to increment</a>.  The <a href="/raw/' + req.params.name + '">API version</a>');
  });
});
